...
const Navbar = ({ setShowSidebar, showSidebar, open, setOpen }) => {
  ...
  const getGMTOffset = () => {
    const offset = -date.getTimezoneOffset() / 60; // Convert minutes to hours and reverse sign
    const sign = offset >= 0 ? '+' : '-';
          ...
          <div className={`flex justify-start gap-[10px] items-center`}>
            ...
            <div>
              <p className="text-sky-400 text-xl gap-1 flex items-center">
                <PiTimerLight className="text-[25px]" /> {date.toLocaleTimeString()} ({timezone} {getGMTOffset()})
              </p>
            </div>
          </div>
